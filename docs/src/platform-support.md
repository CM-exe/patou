...
| Platform                     | `commit-msg` hook enforcement | `install` activation |
| ----------------------------- | ------------------------------ | ---------------------- |
...
## Why the hook itself should work everywhere Git does
...
`.patou/hooks/commit-msg` is plain POSIX shell using only `grep`, `sed`,
and parameter expansion — no GNU-specific flags — it should run
identically wherever Git itself runs hooks. This hasn't been verified on
an actual Windows machine; if you hit an issue, please open one.
